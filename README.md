...
| 6 | **시세 자동 분석 + 적정 FOB 산출** (동급 통계 + baseline + 도착국 보정) | ~50ms |
| 7 | **LLM Wiki 편집 페이지** — 28국 통관 룰 web UI CRUD (yaml seed 외 신규 국가 추가 가능) | 즉시 |
| 8 | **AI 에이전트 채팅** — 자연어 → MCP tool 자동 라우팅 (VIN/통관/매물/제재 등 10 tools) | 패턴 매칭 즉시 / LLM fallback ~3초 |
| 9 | **MCP 서버 추상화** — 기존 비즈니스 로직 10개 tool 로 표준화, `/api/mcp/tools/{list,call}` 외부 client 노출 | — |
